module View exposing (answer, baseForOctave, baseLines, countdown, currentNote, drawLedger, footer, modeButton, modeSelector, noteOffset, overlay, showHint, stats, stave, summary, toPercent, trebleLines, view)
...
import TrebleClef exposing (baseClef, trebleClef)
        ...
        |> List.foldl
            (\n_ ( lines, y_ ) ->
                let
                    ...
                    from =
                        45 - n2 |> String.fromInt

                    to =
                        55 + n2 |> String.fromInt
                in
                ( lines
                    ++ (case baseLines ++ trebleLines |> List.member y of
                            True ->
                                []
                ...
                |> List.map
                    (\o ->
                        div
                            ...
                            [ H.text <| String.fromInt o.octave ]
                    )
             )
...
countdown model =
    let
        w =
            model.time / timeAllowed model.mode * 100 |> String.fromFloat
    in
    div
        [ class "countdown"
        , H.style "width" (w ++ "%")
